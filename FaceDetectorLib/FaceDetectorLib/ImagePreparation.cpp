...
//void ImagePreparation::run(vector<string>& filePaths, int threads) {
...
vector<string> ImagePreparation::getUniqueFolders(vector<string>& images)
{
...
// Returns a vector of nested files in a single catalog
// The paths are absolute ones
vector<string> ImagePreparation::getNonRecursiveFilepaths(string dirPath, vector<string> exts) {
	vector<string> dirs;
	...
	try {
		for (auto& entry : boost::make_iterator_range(boost::filesystem::directory_iterator(p), {})) {
			...
			boost::filesystem::path fileInCatalog(entry);
			string fileExtension = fileInCatalog.extension().string();
			...
			for (auto& extIt : exts) {
				...
				if (fileExtension.compare(extIt) == 0) {
					...
					dirs.push_back(fileInCatalog.string());
				}
			}
		}
	}
